    ...
    const fetchAnalytics = async () => {
      try {
        const [usersRes, invRes, dashRes] = await Promise.all([
          fetch(`${getBaseURL()}/admin/users`, { headers: { Authorization: `Bearer ${getToken()}` } }),
        ...
        if (usersData.success && usersData.users) {
          ...
          const colors = ["var(--admin-info)", "var(--admin-success)", "var(--admin-warning)", "var(--admin-danger)", "#8B5CF6"];
          ...
          const verifiedUsers = usersData.users.filter(u => u.isVerified).length;
          
          let totalInvestors = 0;
          if (invData.success && invData.investments) {
            const uniqueInvestors = new Set(invData.investments.map(i => i.investor?.id));
            totalInvestors = uniqueInvestors.size;
          }

          setFunnelData([
            { label: "Total Signups", count: totalUsers, pct: 100 },
            { label: "KYC Verified", count: verifiedUsers, pct: totalUsers ? Math.round((verifiedUsers/totalUsers)*100) : 0 },
            { label: "Active Investors", count: totalInvestors, pct: totalUsers ? Math.round((totalInvestors/totalUsers)*100) : 0 },
          ]);
        }
        ...
        if (invData.success && invData.investments) {
          ...
          invData.investments.forEach(inv => {
            ...
            monthlyRevenue[monthStr] = (monthlyRevenue[monthStr] || 0) + inv.amount;
          ...
          const revArray = months.map(m => ({
            name: m,
            revenue: monthlyRevenue[m] || 0
          })).filter(item => item.revenue > 0 || item.name === new Date().toLocaleString('default', { month: 'short' })); // show months with data + current month
          
          setRevenueData(revArray.length ? revArray : [{ name: 'Current', revenue: 0 }]);

          const catArray = Object.keys(catFunding).map(cat => ({
            name: cat,
            funding: catFunding[cat]
          })).sort((a, b) => b.funding - a.funding);

          setCategoryData(catArray.length ? catArray : [{ name: 'None', funding: 0 }]);
        }
  ...
  return (
    <AdminLayout title="Analytics">
      <div className="analytics-header">
        ...
        <p className="analytics-subtitle">Real-time deep dive into platform performance and user cohorts.</p>
      </div>
