  ...
  public void lockExpiredProjects() {
    try {
      ...
      List<Project> expiredProjects =
        projectRepository.findByEndDateBeforeAndIsLocked(now, false);
      ...
      if (!expiredProjects.isEmpty()) {
        ...
        System.out.println(
          "✅ Locked and processed investments for " +
            expiredProjects.size() +
            " expired projects."
        );
      }
    } catch (Exception e) {
      System.err.println(
        "❌ Error locking expired projects: " + e.getMessage()
      );
    }
  }
}
