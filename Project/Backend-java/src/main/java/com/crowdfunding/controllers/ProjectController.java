...
public class ProjectController {
      ...
      try {
        ...
        try {
          ...
          return LocalDateTime.parse(
            dateStr.substring(0, Math.min(dateStr.length(), 19))
          );
        }
      }
    } catch (Exception e) {
      throw new RuntimeException(
        "Failed to parse date '" + dateStr + "': " + e.getMessage()
      );
    }
  }
  ...
  public ResponseEntity<?> getAllProjects() {
    try {
      List<Project> projects =
        projectRepository.findByStatusOrderByCreatedAtDesc("approved");
  ...
  public ResponseEntity<?> getUserProjects(
    ...
    try {
      List<Project> projects =
        projectRepository.findByCreatorIdOrderByCreatedAtDesc(
          userDetails.getId()
        );
      return ResponseEntity.ok(projects);
    } catch (Exception e) {
  ...
  public ResponseEntity<?> updateProject(
    ...
    try {
      ...
      if (
        !project.getCreator().getId().equals(userDetails.getId()) &&
  ...
  public ResponseEntity<?> deleteCampaignImage(
    ...
    try {
      ...
      if (
        ...
        error.put(
          "message",
          "Not authorized to delete images from this project"
        );
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
      }
