...
public class ReviewController {
  ...
  public ResponseEntity<?> getCompanyReviews(@PathVariable Long companyId) {
    try {
      List<Review> reviews =
        reviewRepository.findByCompanyIdOrderByCreatedAtDesc(companyId);
  ...
  public ResponseEntity<?> addReview(
    ...
    try {
      ...
      User companyUser = userRepository
        .findById(companyId)
        .orElseThrow(() ->
          new RuntimeException("Target company/user not found")
        );
      ...
      double averageStars = allReviews.isEmpty()
        ? 0.0
        : totalStars / allReviews.size();
